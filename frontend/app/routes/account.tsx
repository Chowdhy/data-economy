...
export default function AccountPage() {
  ...
  useEffect(() => {
    if (!currentUser || currentUser.role_id !== "regulator") return;
    setLogsLoading(true);
    api
      .getAllLogs()
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {currentUser.role_id === "regulator" && (
          <Card>
            <h2 className="text-lg font-semibold text-slate-900">
              Activity log
            </h2>
            <p className="mt-1 text-sm text-slate-600">
              Recent actions taken across all studies.
            </p>
            ...
            {logsLoading ? (
              ...
              <div className="mt-4 divide-y divide-slate-100">
                {logs.map((log) => (
                  <div key={log.log_id} className="py-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    <span className="text-sm text-slate-800">{log.message}</span>
                    {log.study_id && (
                      <span className="text-xs text-slate-500">Study #{log.study_id}</span>
                    )}
